...
use policy_evaluator::callback_requests::{CallbackRequest, CallbackResponse};
...
pub(crate) struct CallbackHandlerBuilder {
...
impl CallbackHandlerBuilder {
    ...
    pub(crate) fn channel_buffer_size(mut self, size: usize) -> Self {
    ...
    pub(crate) fn shutdown_channel(mut self, shutdown_channel: oneshot::Receiver<()>) -> Self {
        self.shutdown_channel = Some(shutdown_channel);
        self
    }

    /// Create a CallbackHandler object
    pub(crate) fn build(self) -> Result<CallbackHandler> {
...
impl CallbackHandler {
    ...
    pub(crate) fn sender_channel(&self) -> mpsc::Sender<CallbackRequest> {
        self.tx.clone()
    }
    ...
    /// `shutdown_channel`.
    pub(crate) async fn loop_eval(&mut self) {
        loop {
            tokio::select! {
                maybe_req = self.rx.recv() => {
                    if let Some(req) = maybe_req {
                        match req {
                            CallbackRequest::OciManifestDigest {
                                image,
                                response_channel,
                            } => {
                                let response =
