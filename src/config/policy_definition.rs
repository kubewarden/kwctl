...
impl fmt::Display for PolicyDefinition {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PolicyDefinition::Individual { id, uri, .. } => write!(f, "Policy {} ({})", id, uri),
            PolicyDefinition::Group { id, .. } => write!(f, "Policy Group {}", id),
        }
    }
}
...
impl TryFrom<AdmissionPolicy> for PolicyDefinition {
    ...
    fn try_from(ap: AdmissionPolicy) -> Result<Self> {
        ...
        Ok(PolicyDefinition::Individual {
            id: ap
                .metadata
                .name
...
impl PolicyDefinition {
    ...
    pub(crate) fn uris(&self) -> HashSet<String> {
        match self {
            PolicyDefinition::Individual { uri, .. } => HashSet::from([uri.clone()]),
            PolicyDefinition::Group { policy_members, .. } => {
